...
此目錄管理研究問題、run 與評估來源。設定實作已依家族搬入 configs 子目錄，舊入口與輸出路徑相容；文件與大型壓縮檔的搬移見 [整理紀錄](../docs/organization_20260924.md)。
...
- [標準化評估表](../reports/tables/evaluations.csv)：由既有 0915 metrics 與兩個 progressive 評估匯入，單位為 TAR 百分比。每列為 checkpoint × evaluation × FAR rule × FAR。
...
- [代表結果總覽](../reports/summary.md)、[run 證據覆蓋](../reports/tables/run_summary.csv)、[來源鏈邊](../reports/tables/lineage.csv)。
- [本機來源快照](local_evidence.json)：119 份 TAR CSV 與設定／run10 文件；[匯入表](../reports/tables/local_evidence.csv)。
- `runs/<run_id>/manifest.json`：由 representatives 與快照生成的 7 份歷史代表 manifest。修改來源後重建，不直接手改。新 run 的自填 manifest 與其 evaluations 不會自動列入結果表，須先加入明確來源 adapter 與校驗。
...
| reduced_nonlinearity | [NL9/NL13](../reports/studies/iresnet50_reduced_nonlinearity_experiment.md)：減少非線性位置 | 保留對照與各層替換記錄 |
| polynomial_conversion | [低階多項式報告](../reports/studies/iresnet50_low_degree_polynomial_practicality_report.md)、[hard containment](../reports/studies/layerwise_poly_hard_containment_report.md)：degree、區間與穩定性 | 歷史對照；run10 原件先保護 |
...
| other_backbones | [PoolFormer](../reports/studies/poolformer_s24_reduced_gelu_experiment.md)、MobileFaceNet、Patch-CNN、NF | 探索線；尚未刪除或實體封存 |
...
```bash
python tools/experiment_registry/registry.py --scan --capture-local
```
...
設定家族遷移與相容性比對已完成；loader 支援巢狀路徑並避免跨設定污染。歷史報告在 `reports/studies/`，根目錄壓縮檔在 `artifacts/archives/`。release builder 使用的權重路徑不變。`needs_review` 留給缺少評估身份或 protocol 的歷史項目，不把自動歸類當作人工審核完成。
