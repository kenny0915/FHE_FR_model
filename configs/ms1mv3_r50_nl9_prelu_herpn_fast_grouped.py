"""Fast grouped NL9 PReLU-to-HerPN conversion in an isolated work directory.
...
This speed-oriented experiment is independent of the original singleton NL9
replacement.  It groups shallow and Layer3 activations, retains singleton
Layer4 transitions, halves BatchNorm recalibration work, and disables the
second full embedding-teacher forward.  Frozen local PReLU teachers remain in
all wrappers and continue relative activation distillation after conversion.
"""
...
config.resume = False
config.output = "work_dirs/ms1mv3_r50_nl9_prelu_herpn_fast_grouped"
...
# Read the same PReLU source checkpoint without touching the output directory
# of the currently running singleton replacement.
...
config.herpn_conversion_groups = (
    # Combine the expensive high-resolution stem and Layer1 activations.
    ("prelu", "layer1.0.prelu"),
    ("layer2.0.prelu",),
    # Convert the long middle stage in forward-ordered pairs.
    ("layer3.0.prelu", "layer3.3.prelu"),
    ("layer3.9.prelu", "layer3.13.prelu"),
    # Preserve conservative singleton transitions nearest the embedding.
    ("layer4.0.prelu",),
    ("layer4.2.prelu",),
)
# Epoch 0 locally fits every zero-blend student.  Each group receives one
# blend epoch and one recovery epoch before the next group starts.
config.herpn_group_epochs = (1, 3, 5, 7, 9, 11)
...
# Conversion completes at epoch 12, followed by four fully polynomial epochs.
config.num_epoch = 16
