...
config.resume = False
config.output = "work_dirs/ms1mv3_iresnet_nf12_fp32"
...
# Twelve blocks, one bounded z*(1+beta*z) product per block. alpha cannot grow
# above 0.2 and beta starts at zero, so the first forward graph is linear.
config.nf_ws_eps = 1e-4
config.nf_alpha_init = 0.05
config.nf_alpha_max = 0.2
config.nf_input_gain_init = 1.0
config.nf_quadratic_scale_max = 0.25
